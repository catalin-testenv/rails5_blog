module ListOpsHelper
  ...
  def list_ops_filter_string(model, column, flt_key)
    ...
    content_tag :div, class: %w(uk-form-row) do
      concat label_tag(flt_key, model.human_attribute_name(column).humanize, class: 'uk-form-label')
      concat(content_tag(:div, class: %w(uk-form-controls)) do
        text_field_tag(flt_key, qp[flt_key], class: 'uk-width')
      end)
    end
  end
  ...
  def list_ops_filter_boolean(model, column, flt_key, yes, no, wildcard)
    qp = request.query_parameters
    content_tag :div, class: ['uk-form-row'] do
      concat label_tag(nil, model.human_attribute_name(column).humanize, class: 'uk-form-label')
      concat(content_tag(:div, class: %w(uk-grid uk-grid-collapse)) do
        concat(content_tag(:div, class: %w(uk-width-1-3)) do
          ...
          concat label_tag("#{flt_key}_", wildcard, class: 'uk-width')
        end)
        concat(content_tag(:div, class: %w(uk-width-1-3)) do
          concat radio_button_tag(flt_key, '0', qp[flt_key] == '0')
          concat label_tag("#{flt_key}_0", no, class: 'uk-width')
        end)
        concat(content_tag(:div, class: %w(uk-width-1-3)) do
          concat radio_button_tag(flt_key, '1', qp[flt_key] == '1')
          concat label_tag("#{flt_key}_1", yes, class: 'uk-width')
        end)
      end)
    end
  end
  ...
  def list_ops_filter_datetime_range(model, column, flt_key, start_key, end_key, start_label, end_label)
    ...
    content_tag :div, class: ['uk-form-row'] do
      ...
      concat(content_tag(:div, class: ['uk-form-controls']) do
        content_tag :div, class: ['uk-grid'] do
          concat(content_tag(:div, class: ['uk-width-1-2']) do
            concat label_tag("#{flt_key}[#{start_key}]", start_label.humanize+':', class: 'uk-form-label')
            concat text_field_tag("#{flt_key}[#{start_key}]", qp[flt_key] && qp[flt_key][start_key], class: 'uk-width', data: {'uk-datepicker' => ''})
          end)
          concat(content_tag(:div, class: ['uk-width-1-2']) do
            concat label_tag("#{flt_key}[#{end_key}]", end_label.humanize+':', class: 'uk-form-label')
            concat text_field_tag("#{flt_key}[#{end_key}]", qp[flt_key] && qp[flt_key][end_key], class: 'uk-width')
          end)
        end
      end)
    end
